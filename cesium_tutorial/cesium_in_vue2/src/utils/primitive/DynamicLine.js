...
function DynamicLine(
    ...
    // let pointA = createPoint(viewer, posA,Cesium.Color.RED);
    // let pointB = createPoint(viewer, posB,Cesium.Color.GREEN);
    ...
    // 确定纹理图片
    let imageUri = './imgs/fromLW/rectangle-T_trans.png';
    // this.imageUri = './imgs/fromLW/rectangle-S_trans.png';

    let primitive = drawDynamicTriangles({
        viewer: viewer,
        imageUri:imageUri,
        typedArray:posArr,
        stArray:stArr,
...
function drawDynamicTriangles(option) {
    /*  option:{
            viewer,
            typedArray,
            stArray,
...
function translateAlongSpecificDirection(posA, posB, distance = 500) {
    ...
    let directionMagnitude = Cesium.Cartesian3.magnitude(direction);//  magnitude vector AB
    ...
    // 4. 根据 得到的degree，获取到 绕Z轴( world ) 的旋转矩阵， 而后更新 posA，posB 的 矩阵
