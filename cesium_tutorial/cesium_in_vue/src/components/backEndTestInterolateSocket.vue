...
export default {
    ...
    methods: {
        ...
        start() {
            ...
            this.socket.emit('startEvent');
             ...
             // 订阅事件
            this.socket.on('getSimDataEvent', function(message) {
                /*
                    {
                        "lat":23.870354,      //纬度
                        "lon":120.004336,     //经度
                ...
                */
                ...
                if (data.lat && data.lon && data.alt) {
                    ...
                    if (this.modelMap.has(data.unitName)) {
                        // console.log(`update--${data.unitName}------data.unitName`, data.unitName);

                        //  a.updatePosition(30,curPosition)
                        // let curTime = (new Date()).getMilliseconds();
                        // let curStap = curTime - this.timeStapMap.get(data.unitName);

                        // console.log("curStap", curStap);
                        ...
                        if (data.time > 0) {
                            // console.log(`update--${data.unitName}------data.time`, data.time);
                            if (data.unitName == 'F-16') {
                                console.log(`${data.unitName}-${data.time.toFixed(1)}--curPosition:`, curPosition);
                            }
                            this.modelMap.get(data.unitName).updatePosition(0.11,curPosition);
                        }
                        // this.labelMap.get(data.unitName).updatePosition(0.1,curPosition);
                        // console.log("this.modelMap.get(data.unitName).computeModelMatrix", this.modelMap.get(data.unitName).computeModelMatrix);
                        ...
                        // console.log(`init--${data.unitName}------data.unitName`, data.unitName);

                        let curModel = new addEntityModel(this.viewer, curPosition);
                        // console.log(`init--${data.unitName}:`, curModel.entity);
                        // console.log(`init--${data.unitName}:`, curModel.entity.orientation);
                        // console.log(`init--${data.unitName}:`, curModel.entity.orientation._velocityVectorProperty);
                        /*
                            _definitionChanged: Event
                            _ellipsoid: Ellipsoid
                            _subscription: undefined
                            _velocityVectorProperty: VelocityVectorProperty
                        */
            ...
            // console.log("this.timeStap", this.timeStap);
        },
        stop() {
        ...
        testAddMultiModel() {
            ...
            let curPosition2 = Cesium.Cartesian3.fromDegrees( lon - 10.8, lat, 5000.0 );
            let b =new addEntityModel(this.viewer, curPosition2)
            let curPosition3 = Cesium.Cartesian3.fromDegrees( lon - 21.6, lat, 5000.0 );
            ...
            setInterval(() => {
                ...
                let curPosition = Cesium.Cartesian3.fromDegrees( lon, lat, 50000.0 );
                let curPosition2 = Cesium.Cartesian3.fromDegrees( lon - 10.8, lat, 50000.0 );
                let curPosition3 = Cesium.Cartesian3.fromDegrees( lon - 21.6, lat, 50000.0 );
    ...
    created() {
        ...
        // init socket.io
        // this.socket = io("ws://192.168.1.9:9211");// http://192.168.1.9:9211
        this.socket = io.connect('http://192.168.1.9:9211');
