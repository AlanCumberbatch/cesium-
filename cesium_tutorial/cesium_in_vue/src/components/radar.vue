...
export default {
    ...
    methods: {
        ...
        rotateEntity(instance) {//已经被收集
            // let heading = this.heading;
            let heading = 10.0;
            let position = instance.position._value;

            //  这个方法会导致 整个雷达模型渲染不出来，但是又不报错
            // this.viewer.clock.onTick.addEventListener(() => {
            //     heading += 4.0;
            //     let pos = Cesium.Transforms.headingPitchRollQuaternion(position, new Cesium.HeadingPitchRoll(heading * Math.PI / 180, 0.0, 0.0));
        ...
        create_first_RadarByElliposidFor2() {
            let position = Cesium.Cartesian3.fromDegrees(-80, 35, 5000.0);//飞机模型position/模型扫描区的positoin

            let angleOfHollow = 40.0;
            let angleOfSector = 90.0 - angleOfHollow; // 扇形的角度
            // let radius = this.radius; //200000.0; //球体半径/扇形侧边距/雷达扫描半径
            let radius = 10000.0; //球体半径/扇形侧边距/雷达扫描半径
            let innerRadius = 5.0;
            ...
            let ellipsoid = this.viewer.entities.add({
                name: "Dome with top cut out",
                position: position,
                orientation: Cesium.Transforms.headingPitchRollQuaternion(position, new Cesium.HeadingPitchRoll(0 * Math.PI / 180, 0.0, 0.0)),
                ellipsoid: {
                    radii: new Cesium.Cartesian3(radius, radius, radius), // 扇形半径
                    innerRadii: new Cesium.Cartesian3( innerRadius, innerRadius, innerRadius ), //内半径
                    minimumCone: Cesium.Math.toRadians(angleOfSector),
                    ...
                    minimumClock: Cesium.Math.toRadians(0),
                    maximumClock: Cesium.Math.toRadians(120),
    ...
    mounted() {
        ...
        */
        ...
        // this.create_first_RadarByElliposidFor2();
    },
};
</script>
