...
function createCustomMaterial(options) {
    // let imgUrl = options && options.img ? options.img : "./imgs/colors.png";// 默认文件路径可以这么写的原因是： 相关 img 都被放到了 public 里
    let type = options && options.type ? options.type : 'PolylineTrailLink';// type的命名规则： polyline/polygon/图元名称 + 描述(根据具体特性)
    ...
    let MaterialPropertyType = `${type}MaterialProperty`;// PolylineTrailLinkType
    ...
    function MaterialProperty(color, duration,d) {
    ...
    let timeStamp = 0
    MaterialProperty.prototype.getValue = function (time, result) {
        ...
        if (timeStamp <= 600) {
            ...
            result.color = Cesium.Property.getValueOrClonedDefault(this.color, time, null, result.color);
        } else {
            // result.color = Cesium.Color.CHOCOLATE;
            result.color = Cesium.Property.getValueOrClonedDefault(Cesium.Color.CHOCOLATE, time, null, result.color);
            timeStamp = 0;
        }
    ...
    // MaterialProperty.prototype.equals = function (other) {
    //     return this === other || (other instanceof MaterialProperty && Property.equals(this._color, other._color))
    // }
    MaterialProperty.prototype.equals = function (other) {
        return (
            this === other ||
    ...
    Cesium[MaterialPropertyType] = MaterialProperty;
